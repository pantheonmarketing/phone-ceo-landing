...
class VercelBlobAuditStore extends EventEmitter {
  ...
  async list({ statuses, limit = 100 } = {}) {
    ...
    const pageLimit = unlimited ? 1000 : Math.min(1000, Math.max(1, limit * 3))
    const records = []
    let cursor
    do {
      const page = await list(this._options({ prefix: `${this.prefix}/records/`, limit: pageLimit, cursor }))
      for (const pathname of page.blobs.map(blob => blob.pathname)) {
        const match = pathname.match(/((?:FBA|WBA)-[A-F0-9]{8})\.json$/i)
        if (!match) continue
        const record = await this.get(match[1])
        if (record && (!allowed || allowed.has(record.status))) records.push(record)
        if (!unlimited && records.length >= limit) break
      }
      cursor = page.hasMore ? page.cursor : undefined
    } while (cursor && (unlimited || records.length < limit))
  ...
  async claimNext(workerId, now = new Date(), { auditTypes } = {}) {
    const queued = (await this.list({ statuses: ['queued'], limit: null }))
      .filter(record => !auditTypes || new Set(auditTypes).has(auditTypeOf(record)))
      .sort((a, b) => a.requestedAt.localeCompare(b.requestedAt))
    for (const record of queued) {
      try {
        return await this.update(record.auditId, current => {
          if (current.status !== 'queued') throw new VersionConflictError()
          return transitionClaimedAudit(current, workerId, now)
        }, record.version)
      } catch (error) {
