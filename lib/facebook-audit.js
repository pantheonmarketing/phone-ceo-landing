...
function classifyFacebookReply(value, { customerQuestion = '' } = {}) {
  ...
  const genericReplyWords = new Set(['a', 'an', 'and', 'can', 'do', 'does', 'for', 'good', 'have', 'help', 'i', 'it', 'of', 'offer', 'provide', 'sure', 'the', 'to', 'we', 'will', 'with', 'yes', 'yeah', 'yep'])
  const replyHasSpecificContent = /[$€£฿\d]/.test(text) || /\b(?:days?|hours?|weeks?|months?|monday|tuesday|wednesday|thursday|friday|saturday|sunday|morning|afternoon|evening|weekend|available|availability|included|fee|fees|rate|rates|price|prices|cost|costs|refund|refundable|cancel|cancellation)\b/i.test(text) ||
    (text.match(/[a-z0-9]+/gi) || []).some(word => !questionWords.has(word.toLowerCase()) && !genericReplyWords.has(word.toLowerCase()))
  ...
  if (/do you|can you|offer|provide|have you/.test(question)) {
    answerChecks.push(() => /\b(yes|no|we do|we don't|we can|we cannot|we offer|we provide)\b/i.test(text) && replyHasSpecificContent)
  }
  if (asksCancellation) {
    questionFacets.push('cancellation')
    cancellationAnswerCheck = () => replyAddressesCancellation && (hasCancellationProcedure || hasCancellationDetail)
