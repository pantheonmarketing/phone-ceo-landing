...
function classifyFacebookReply(value, { customerQuestion = '' } = {}) {
  ...
  const hasClearNextAction = hasBookingCta || /\b(click|send|share|visit|come in|reply with|message us|call|confirm|available|availability|slot|open)\b/i.test(text)

  let answersQuestion = false
  if (/availab|open|slot|when|this week|today|tomorrow/.test(question)) {
    answersQuestion ||= /\b(available|availability|open|closed|booked|slot|monday|tuesday|wednesday|thursday|friday|saturday|sunday|today|tomorrow)\b/i.test(text)
  }
  if (/cost|price|how much|rate|fee/.test(question)) {
    answersQuestion ||= /(?:[$€£฿]\s?\d|\d[\d,.]*\s?(?:usd|eur|gbp|thb|dollars?|baht)|\b(?:price|cost|rate|fee|from|per person)\b)/i.test(text)
  }
  if (/do you|can you|offer|provide|have you/.test(question)) {
    answersQuestion ||= /\b(yes|no|we do|we don't|we can|we cannot|we offer|we provide)\b/i.test(lower)
  }

  const isUseful = Boolean(text) && !isAutoAcknowledgement && (answersQuestion || hasQualificationQuestion || hasBookingCta || hasClearNextAction)
