...
function classifyFacebookReply(value, { customerQuestion = '' } = {}) {
  ...
  if (asksCancellation) {
    answersQuestion ||= replyAddressesCancellation && /\b(policy|can|able|allow|allowed|fee|charge|call|contact|email|step|process)\b/i.test(lower)
  }

  const isUseful = Boolean(text) && !isAutoAcknowledgement && (
    answersQuestion ||
    (actionSeekingQuestion && actionIsRelevant && (hasQualificationQuestion || hasBookingCta || hasExplicitActionPhrase))
  )
