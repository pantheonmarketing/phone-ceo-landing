...
const { transitionWebsiteAudit } = require('./website-audit-state')

const AUDIT_ID_PATTERN = /^(?:FBA|WBA)-[A-F0-9]{8}$/i

function auditTypeOf(record) {
  return record?.auditType === 'website' || /^WBA-/i.test(String(record?.auditId || '')) ? 'website' : 'facebook'
}

function transitionClaimedAudit(record, workerId, now) {
  if (auditTypeOf(record) === 'website') {
    return transitionWebsiteAudit(record, 'starting', { workerId, message: 'Worker claimed queued website audit' }, now)
  }
  return transitionAudit(record, 'starting', { workerId, message: 'Worker claimed queued audit' }, now)
}

function claimTypeAllowed(record, auditTypes) {
  return !auditTypes || new Set(auditTypes).has(auditTypeOf(record))
}
...
class MemoryAuditStore extends EventEmitter {
  ...
  async claimNext(workerId, now = new Date(), { auditTypes } = {}) {
    return this._locked(async () => {
      const queued = [...this.records.values()]
        .filter(record => record.status === 'queued' && claimTypeAllowed(record, auditTypes))
        .sort((a, b) => a.requestedAt.localeCompare(b.requestedAt))[0]
      if (!queued) return null
      const claimed = transitionClaimedAudit(queued, workerId, now)
...
class FileAuditStore extends EventEmitter {
  ...
  _recordPath(auditId) {
    if (!AUDIT_ID_PATTERN.test(String(auditId))) throw new Error('Invalid audit ID')
  ...
  async _listUnlocked({ statuses, limit = 100 } = {}) {
    await this._init()
    const allowed = statuses ? new Set(statuses) : null
    const names = (await fs.readdir(this.recordsDirectory)).filter(name => /^(?:FBA|WBA)-[A-F0-9]{8}\.json$/i.test(name))
  ...
  async claimNext(workerId, now = new Date(), { auditTypes } = {}) {
    return this._locked(async () => {
      const queued = (await this._listUnlocked({ statuses: ['queued'], limit: null }))
        .filter(record => claimTypeAllowed(record, auditTypes))
        .sort((a, b) => a.requestedAt.localeCompare(b.requestedAt))[0]
      if (!queued) return null
      const claimed = transitionClaimedAudit(queued, workerId, now)
      claimed.version = queued.version + 1
  ...
  async getEvidence(reference) {
    ...
    if (!/^evidence\/(?:FBA|WBA)-[A-F0-9]{8}\/[a-zA-Z0-9._-]+$/.test(normalized)) throw new Error('Invalid evidence reference')
