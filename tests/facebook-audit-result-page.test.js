...
test('approved light report centers the score, measured gap, and friendly journey', () => {
  ...
  assert.match(page, /@keyframes journeyPulse/)
  assert.match(page, /\.journey-step\.active \.step-icon\s*\{[^}]*animation:\s*journeyPulse/is)
  assert.match(page, /\.journey-step\.active::after\s*\{[^}]*animation:\s*journeyConnectorFlow/is)
  assert.match(page, /setAttribute\("aria-current", "step"\)/)
  assert.match(page, /prefers-reduced-motion[\s\S]*\.journey-step\.active \.step-icon,[\s\S]*animation:\s*none !important/i)
